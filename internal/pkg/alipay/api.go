...
// Pay APP跟wap支付合集，内部会对入参做统一的检验跟格式化操作
func (a *AliPay) Pay(ctx context.Context, args *PayRequest) (*PayResponse, error) {
	...
	if args.PayType == PayTypeWap && args.QuitURL == "" {
...
//外部商户APP唤起快捷SDK创建订单并支付 https://docs.open.alipay.com/api_1/alipay.trade.app.pay
...
// TradeWapPay 手机网站支付接口2.0
// 外部商户创建订单并支付 https://docs.open.alipay.com/api_1/alipay.trade.wap.pay
...
//用于交易创建后，用户在一定时间内未进行支付，可调用该接口直接将未付款的交易进行关闭  https://docs.open.alipay.com/api_1/alipay.trade.close
...
/*
TradeCancel 支付交易返回失败或支付系统超时，调用该接口撤销交易。如果此订单用户支付失败，支付宝系统会将此订单关闭；
如果用户支付成功，支付宝系统会将此订单资金退还给用户。 注意：只有发生支付系统超时或者支付结果未知时可调用撤销，其他正常支付的单如需实现相同功能请调用申请退款API。提交支付交易后调用【查询订单API】，没有明确的支付结果再调用【撤销订单API】。
...
/*
BillDownloadURLQuery 查询账单下载地址接口
